...
const MaterialReductionModal = ({ isOpen, onClose, onSaveSuccess, orderId, materialsList }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!response.ok) throw new Error(data.message || 'Gagal memulai produksi');
  ...
  return (
    <>
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
        ...
        >
          <h2 className="text-2xl font-bold text-gray-800 mb-6">Mulai Produksi (Pengurangan Stok)</h2>
          <form onSubmit={handleSubmit} className="space-y-4">
            
            <div className="max-h-[40vh] overflow-y-auto space-y-4 pr-2">
              {items.map((item, index) => (
                <div key={index} className="grid grid-cols-12 gap-3 p-3 bg-gray-50 rounded-lg relative">
                  {/* Kolom 1: Bahan */}
                  <div className="col-span-5">
                    <label className="block text-xs font-medium text-gray-700">Bahan Baku</label>
                    <select
                      name="material_id"
                    ...
                    >
                      ...
                      {materialsList.map(m => (
                        <option key={m.id} value={m.id}>
                          {m.name} ({m.size || 'N/A'}) - Sisa: {m.quantity} {m.unit}
                        </option>
                      ))}
                    </select>
                  </div>
