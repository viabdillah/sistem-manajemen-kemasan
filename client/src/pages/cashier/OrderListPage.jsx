...
import { BsPlusCircleFill, BsSearch, BsDownload, BsEyeFill, BsCashCoin, BsTrash } from 'react-icons/bs';
...
const getOrderStatusChip = (status) => {
  switch (status) {
    case 'pending': return <span className="px-3 py-1 text-xs font-semibold rounded-full bg-gray-100 text-gray-700">Pending</span>;
    ...
    case 'completed': return <span className="px-3 py-1 text-xs font-semibold rounded-full bg-green-100 text-green-800">Siap Ambil</span>;
    case 'cancelled': return <span className="px-3 py-1 text-xs font-semibold rounded-full bg-red-100 text-red-800 line-through">Batal</span>;
    default: return <span className="px-3 py-1 text-xs font-semibold rounded-full bg-gray-100 text-gray-700">{status}</span>;
  }
};
const getPaymentStatusChip = (status) => {
  switch (status) {
    case 'paid': return <span className="text-xs font-medium text-green-600">Lunas</span>;
    case 'pending': return <span className="text-xs font-medium text-red-600">Belum Lunas</span>;
    default: return <span className="text-xs font-medium text-gray-600">{status}</span>;
  }
};
      ...
      <table className="min-w-full divide-y divide-gray-200">
         <thead className="bg-gray-50">
          <tr>
            ...
            <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Tgl. Pesan</th>
        ...
        <tbody className="bg-white divide-y divide-gray-200">
          {orders.map((order) => {
            const isReadyForPayment = order.order_status === 'completed' && order.payment_status === 'pending';
            const isCancellable = order.order_status !== 'completed' && order.order_status !== 'cancelled';
            return (
              <tr key={order.order_id} className="hover:bg-gray-50">
                ...
                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-600">
                  {new Date(order.order_date).toLocaleDateString('id-ID', { day: '2-digit', month: 'short' })}
                </td>
                <td className="px-6 py-4 whitespace-nowrap">{getPaymentStatusChip(order.payment_status)}</td>
                <td className="px-6 py-4 whitespace-nowrap">{getOrderStatusChip(order.order_status)}</td>
...
const OrderListPage = () => {
  ...
  const summaryData = useMemo(() => {
    return {
      ...
      awaitingDesigner: orders.filter(o => o.order_status === 'design_queue' || o.order_status === 'design_review').length,
      processProduction: orders.filter(o => o.order_status === 'production_queue' || o.order_status === 'in_production').length,
      takeOrders: orders.filter(o => o.order_status === 'completed' && o.payment_status === 'pending').length,
    };
  }, [orders]);
  ...
  // 3. Perbarui 'handleProcessPayment'
    ...
    toast(
      (t) => (
        <div className="flex flex-col gap-3">
          <p>Anda yakin ingin membatalkan pesanan <strong>{order.order_number}</strong>?</p>
            ...
            <button
              onClick={async () => {
                ...
                try {
                  const token = await getAuthToken();
                  // Perbarui panggilan fetch
                  const response = await fetch(`${API_URL}/api/admin/orders/${order.order_id}/cancel`, {
                    method: 'PUT',
                    headers: { Authorization: `Bearer ${token}` }
                  });
                  const data = await response.json();
  ...
  return (
    <div className="p-4 md:p-6 animate-fade-in space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <StatusCard title="New Orders" count={summaryData.newOrders} percentageChange={0} />
        <StatusCard title="Awaiting Desainer" count={summaryData.awaitingDesigner} percentageChange={0} />
        <StatusCard title="Process Production" count={summaryData.processProduction} percentageChange={0} />
        <StatusCard title="Take Orders" count={summaryData.takeOrders} percentageChange={0} />
      </div>
