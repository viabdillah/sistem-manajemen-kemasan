...
const AddCustomerPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setFormData({
        name: '', email: '', whatsapp_number: '', production_address: ''
      });
      fetchCustomers(); 
    } catch (error) {
  ...
  return (
    <div className="p-4 md:p-6 animate-fade-in">
      <h1 className="text-3xl font-bold text-gray-800 mb-6">Tambah Pelanggan Baru</h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2">
          <div className="bg-white shadow-lg rounded-xl p-6 md:p-8">
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              {/* ... (Input Alamat) ... */}
              <div>
                <label htmlFor="production_address" className="block text-sm font-medium text-gray-700 mb-1">
                  Alamat Produksi (Opsional)
                </label>
                <textarea
                  name="production_address" id="production_address"
                  value={formData.production_address} onChange={handleChange}
                  rows={4}
        ...
        {/* Kolom Kanan: Daftar Pelanggan (tidak berubah) */}
        <div className="lg:col-span-1">
          <div className="bg-white shadow-lg rounded-xl p-6 md:p-8">
            <h2 className="text-2xl font-semibold text-gray-700 mb-4">Daftar Pelanggan</h2>
            ...
            <div className="space-y-4 max-h-[500px] overflow-y-auto pr-2">
              {loadingList ? (
                ...
                <p className="text-gray-500">
                  {searchTerm ? 'Pelanggan tidak ditemukan.' : 'Belum ada pelanggan terdaftar.'}
                </p>
              ) : (
                filteredCustomers.map((customer) => (
                  <div key={customer.id} className="flex items-center gap-4 p-3 bg-gray-50 rounded-lg border border-gray-200">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-gray-600">{customer.whatsapp_number}</p>
                    </div>
                  </div>
                ))
              )}
