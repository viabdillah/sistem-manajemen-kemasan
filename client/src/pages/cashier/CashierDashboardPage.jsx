...
const CashierDashboardPage = () => {
  ...
  // --- 3. Perbarui 'useEffect' dan 'fetch' ---
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        // Gunakan API_URL, bukan localhost
        const response = await fetch(`${API_URL}/api/kasir/dashboard-stats`, {
  ...
  return (
    <div className="p-4 md:p-6 animate-fade-in space-y-6">
      ...
      {/* Tombol Pintas (tidak berubah) */}
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <Link
          to="/kasir/pesanan/tambah"
          className="block p-8 bg-blue-600 text-white rounded-xl shadow-lg hover:bg-blue-700 transition-all transform hover:scale-105"
        >
          <div className="flex items-center gap-4">
            <BsPlusCircleFill size={40} />T{" "}
            <span className="text-2xl font-bold">Buat Pesanan Baru</span>
          </div>
        </Link>
        <Link
          to="/kasir/pelanggan/tambah"
          className="block p-8 bg-gray-700 text-white rounded-xl shadow-lg hover:bg-gray-800 transition-all transform hover:scale-105"
        >
          <div className="flex items-center gap-4">
      ...
      {/* Daftar Pelanggan Terbaru (tidak berubah) */}
      <div className="bg-white p-6 rounded-xl shadow-lg">
        <div className="flex justify-between items-center mb-4">
          <h2 className="text-xl font-semibold text-gray-800">
            Pelanggan Terbaru Didaftarkan
          </h2>
          <Link
            to="/kasir/pelanggan/tambah" // Seharusnya ke /kasir/pelanggan
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {recentCustomers.length === 0 ? (
                <tr>
                  ...
                  >
                    Belum ada pelanggan yang didaftarkan.
                  </td>
                </tr>
              ) : (
