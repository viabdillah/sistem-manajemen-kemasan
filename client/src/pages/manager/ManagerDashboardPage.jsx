...
const formatCurrency = (number) =>
  new Number(number).toLocaleString("id-ID", {
    style: "currency",
    currency: "IDR",
...
const ManagerDashboardPage = () => {
  ...
  useEffect(() => {
    const fetchAllData = async () => {
      ...
      try {
        ...
        // 2a. Fetch Statistik
        const statsRes = fetch(`${API_URL}/api/manager/dashboard-stats`, {
          headers,
        });
        // 2b. Fetch Chart
        const chartRes = fetch(`${API_URL}/api/manager/charts/sales`, {
          headers,
        });
        ...
        // (Logika pemrosesan chart sudah benar)
  ...
  // --- Sisa JSX di bawah ini sudah benar ---
  return (
    <div className="p-4 md:p-6 animate-fade-in space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        <StatCard
          ...
          value={formatCurrency(stats.total_pemasukan)}
          icon={<BsArrowUpRight />}
          colorClass="bg-green-100 text-green-800"
        />
        <StatCard
          title="Total Pengeluaran (Operasional)"
          value={formatCurrency(stats.total_pengeluaran)}
          icon={<BsArrowDownRight />}
          colorClass="bg-red-100 text-red-800"
        />
        <StatCard
          title="Pesanan Aktif (Desain/Produksi)"
        ...
        <StatCard
          ...
          value={formatCurrency(stats.total_biaya_bahan)}
          icon={<BsBoxSeam />}
