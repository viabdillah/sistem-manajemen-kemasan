...
const MaterialHistoryPage = () => {
  ...
  const chartRef = useRef(null); // <-- Ganti React.useRef
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      // Perbarui fetch chart (Asumsi token ini juga punya izin Manajer/Admin)
      const chartRes = fetch(`${API_URL}/api/manager/charts/material-usage`, {
        headers,
      });
      ...
      if (chartResponse.ok) {
        ...
        // Jangan lempar error agar riwayat tetap tampil
        console.error("Gagal mengambil data chart bahan");
      ...
      <div className="bg-white p-6 rounded-xl shadow-lg">
        <div className="flex justify-between items-center mb-4">
          <h2 className="text-xl font-semibold text-gray-800">
            Top 5 Bahan Terpakai (Pengeluaran)
          </h2>
          <button
            ...
            {filteredHistory.map((item) => (
              ...
              <tr key={item._id} className="hover:bg-gray-50">
                ...
                <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">
                  {item.name} ({item.material_type || "N/A"} -{" "}
                  {item.size || "N/A"})
                </td>
